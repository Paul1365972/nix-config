{ inputs, ... }:
{
  ...
  den.aspects.rpi.nixos =
    { lib, pkgs, ... }:
    {
      ...
      # doesn't define (nvmd/nixos-raspberrypi#201); graft them on (no rebuild). The option's
      # `apply` re-invokes kernel.override, so the graft must wrap override to survive it.
      boot.kernelPackages =
        let
